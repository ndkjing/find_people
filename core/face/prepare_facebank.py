...
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath('__file__'))), 'insight_face'))
import argparse
...
try:
    ...
    from face.retinaface.data import cfg_mnet, cfg_re50
    from face.retinaface.layers.functions.prior_box import PriorBox
    from face.retinaface.utils.nms.py_cpu_nms import py_cpu_nms
    from face.retinaface.utils.box_utils import decode, decode_landm
    from face.retinaface.models.retinaface import RetinaFace
...
try:
    ...
    from insight_model import l2_norm
    from insight_utils import load_facebank, draw_box_name, prepare_facebank
except:
    from face.insightface.config import get_config
    from face.insightface.Learner import face_learner
    from face.insightface.insight_utils import load_facebank, draw_box_name, prepare_facebank
    from face.insightface.insight_model import l2_norm
...
# embedding 超参数
prepare_image_path = '/Data/jing/face/facebank'  # facebank路径 按名称进行命名  提取特征后在该路径保存name和feature

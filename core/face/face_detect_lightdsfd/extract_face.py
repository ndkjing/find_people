...
class TestBaseTransform:
    ...
    def test_base_transform(self, image, mean):
    ...
    def __call__(self, image, boxes=None, labels=None):
        return self.test_base_transform(image, self.mean), boxes, labels
...
class ExteactFace:
    def __init__(self):
        ...
        self.videos_path_uuid_jsonpath = base_config.videos_path_uuid_jsonpath
        self.thresh = 0.9
    ...
    def process_video(self):
        with open(self.videos_path_uuid_jsonpath,'r') as f:
            videos_path_uuid = json.load(f)
        for video_path,(uuid_code,have_image,extracted,) in tqdm(videos_path_uuid.items()):
            # 处理文件夹中图片
            if have_image:
                ...
                if images_name is [] or len(images_name) ==0:
                    raise Exception('have images path do not have any image like png jpg bmp')
                for image_name in images_name:
                    image_path = os.path.join(os.path.split(video_path)[0],image_name)
                    # 传递给图片处理函数
                    out_images = self.process_image(image_path=image_path)
                    for i, (out_image,face_confi) in enumerate(out_images):
                        write_image_dir = os.path.join(base_config.extract_face_dir, uuid_code)
                        if not os.path.exists(write_image_dir):
                            os.mkdir(write_image_dir)
                        write_image_path = os.path.join(write_image_dir, str(i) + '_' + str(face_confi) + '.jpg')
                        cv2.imwrite(write_image_path,out_image)
            ...
            while True:
                ...
                if current_frame%interval==0:
                    ...
                    if len(out_images)!=0:
                        interval=fps//5
                        patient=10
                    if interval==fps//5 and len(out_images)==0:
                        patient-=1
                    if patient<=0:
                        interval=fps
                ...
                current_frame+=1
                # 处理完视频后 更新视频为已处理标志位
            videos_path_uuid.update({video_path: [uuid_code, have_image, True]})
            with open(base_config.videos_path_uuid_jsonpath, 'w') as f:

...
from core.face.face_detect_lightdsfd.operations import *
...
# import genotypes
import time
from core.face.face_detect_lightdsfd.dataset import *
from core.face.face_detect_lightdsfd.layers import *
from core.face.face_detect_lightdsfd.bi_fpn import BiFPN_PRIMITIVES, BiFPN_Neck_From_Genotype
from core.face.face_detect_lightdsfd.layers.box_utils import match_anchors, decode, encode
import numbers
        ...
        if isinstance(priors, list) or isinstance(priors, tuple):
            ...
            if face_priors.shape[0] != batch:
                ...
                if use_cuda:
                    ...
                    face_priors = (
                        face_priors.view([1] + list(face_priors.shape)).expand([batch] + list(face_priors.shape)).cpu()
                    )
                    head_priors = (
                        head_priors.view([1] + list(head_priors.shape)).expand([batch] + list(head_priors.shape)).cpu()
                    )
                    body_priors = (
                        body_priors.view([1] + list(body_priors.shape)).expand([batch] + list(body_priors.shape)).cpu()
                    )
